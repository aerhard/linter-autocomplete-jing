...
var messageRegex = /^((.*?):)?((\d+):)?((\d+): )?((error|fatal|warning): )(.*)$/;
var jars = {
...
var parseMessage = function parseMessage(textEditor, schema) {
  return function (str) {
    ...
    var effectiveLine = typeof line === 'undefined' && text.startsWith('file not found') ? schema.line : Number(line) - 1;

    var filePath = textEditor.getPath();
    var range = systemId === filePath ? helpers.rangeFromLineNumber(textEditor, effectiveLine) : undefined;
...
function runJing(textEditor, schema) {
  ...
  return helpers.exec(localConfig.javaExecutablePath, params, options).then(function (messageString) {
