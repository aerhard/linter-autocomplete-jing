...
var getJars = function getJars(lang) {
  return [].concat(lang === 'xsd' ? [jars.xerces] : [], lang === 'sch.15' || lang === 'sch.iso' ? [jars.saxon] : [], localConfig.xmlCatalogs ? [jars.resolver] : [], [jars.jing]);
};
...
function runJing(textEditor, schema) {
  ...
  }).join(classPathDelimiter), 'com.thaiopensource.relaxng.util.Driver', '-S', '-r'].concat(schema.lang === 'xsd' && schema.path === '-' ? ['-x'] : [], schema.lang === 'dtd' ? ['-v'] : [], schema.lang === 'rnc' ? ['-c'] : [], localConfig.xmlCatalogs ? ['-C', localConfig.xmlCatalogs] : [], [schema.path, xmlPath]);
...
function getSchemaRefs(textEditor) {
  ...
  return new Promise(function (resolve) {
    ...
    var onDoctype = function onDoctype() {
      schemata.push({
        lang: 'dtd',
        line: saxParser.line,
        path: '-'
    ...
    var onOpenTag = function onOpenTag(node) {
      ...
      var hasSchemaLocation = getXsiNamespacePrefixes(node.attributes).some(function (prefix) {
