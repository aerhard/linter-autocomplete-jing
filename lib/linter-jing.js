...
var parseMessage = function parseMessage(textEditor, schema) {
  return function (str) {
    ...
    var textPrefix = level === 'warning' ? 'Schema parser warning: ' : 'Could not process schema / catalog: ';
...
var getJars = function getJars(lang) {
  return [].concat(lang === 'xsd' ? [jars.xerces] : [], lang === 'sch.15' || lang === 'sch.iso' ? [jars.saxon] : [], localConfig.xmlCatalog ? [jars.resolver] : [], [jars.jing]);
};
...
function runJing(textEditor, schema) {
  ...
  }).join(classPathDelimiter), 'com.thaiopensource.relaxng.util.Driver', '-S', '-r'].concat(schema.lang === 'xsd' && schema.path === '-' ? ['-x'] : [], schema.lang === 'dtd' ? ['-v'] : [], schema.lang === 'rnc' ? ['-c'] : [], localConfig.xmlCatalog ? ['-C', localConfig.xmlCatalog] : [], [schema.path, xmlPath]);
