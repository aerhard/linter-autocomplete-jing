...
var mapWithKey = _fp.map.convert({ cap: false });
...
var parse = (0, _fp.flow)((0, _fp.map)((0, _fp.flow)((0, _fp.update)('test', function (_ref6) {
  ...
  return matchers.length ? (0, _fp.allPass)(matchers) : function () {
    return false;
  };
}), function (rule) {
  var newOutcome = {};
  ...
  var basePath = _path2.default.dirname(settingsPath);

  if (outcome.xmlCatalog) {
    newOutcome.xmlCatalog = _path2.default.resolve(basePath, outcome.xmlCatalog);
  }
  if (outcome.schemaProps) {
    newOutcome.schemaProps = outcome.schemaProps.map(function (_ref7) {
      var schemaPath = _ref7.path;
      return {
        path: _path2.default.resolve(basePath, schemaPath)
      };
    });
  }
  return (0, _fp.merge)(rule, { outcome: newOutcome });
})), sortByPriority);
