...
var buildElementSuggestion = function buildElementSuggestion(replacementPrefix, addSuffix) {
  return function (_ref7) {
    ...
    if (addSuffix) {
      ...
      var nsSnippet = nsUri ? ['xmlns="' + nsUriSnippet + '"'] : [];
...
var getTagNamePrefix = function getTagNamePrefix(precedingLineText) {
  var match = precedingLineText.match(_regex2.default.tagName);
...
var buildHeaders = function buildHeaders(editorPath, catalogPath, _ref10, type, fragment) {
  ...
  return ['A', type, fragment || '', 'r', editorPath, catalogPath || '', lang + ' ' + (schemaPath || '')];
};
...
var getSuggestions = function getSuggestions(sharedConfig, suggestionOptions) {
  ...
  return serverProcessInstance.sendRequest(headers, body, port).then((0, _fp.flow)(JSON.parse, (0, _fp.filter)(filterFn), (0, _fp.map)(builderFn), _fp.compact)).catch(function () {
    return [];
  });
};
...
var getElementSuggestions = function getElementSuggestions(sharedConfig, tagNamePrefix) {
  var options = sharedConfig.options;
  var editor = options.editor;
  var bufferPosition = options.bufferPosition;


  var body = editor.getTextInBufferRange([[0, 0], [bufferPosition.row, bufferPosition.column - tagNamePrefix.length - 1]]);
...
var suggest = function suggest(options, _ref15) {
  ...
  return function (_ref16) {
    ...
    var tagNamePrefix = getTagNamePrefix(precedingLineText);
